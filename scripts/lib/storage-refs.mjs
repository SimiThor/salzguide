 ...
 * tour_point_audio bleiben bis 0069 stehen und werden hier weiter geschützt.
 */
export async function collectTourAudioPaths(sel) {
  ...
  // BEIDE Pfade je Zeile: Volldatei UND Kostprobe. Bis 15.09.2026 stand hier nur
  // audio_url, und der woechentliche Waisen-Sweep hat daraufhin alle sieben Kostproben der
  // Runde A aus dem Bucket geloescht: Die DB zeigte auf Dateien, die es nicht mehr gab, und
  // der Fahrbildschirm zeigte Gaesten ohne Pro ein Schloss statt der 20 Sekunden gratis.
  // Wer hier eine Spalte oder Tabelle vergisst, loescht Inhalte, keine Waisen.
  for (const f of await sel("tour_point_voice_files", "point_id,lang,voice_id,audio_url,teaser_url")) {
    push(f.audio_url);
    push(f.teaser_url);
  }
  // Altspalten (vor 0068). Bis 0069 mitlesen: Ein Rueckweg, der Dateien verliert, ist keiner.
